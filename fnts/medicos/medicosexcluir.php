...
# printf("$bloco<br>\n");
iniciapagina(TRUE,"Médicos","Excluir");
$_REQUEST['salto']=$_REQUEST['salto']+1;
montamenu("M&eacute;dicos","Excluir","$_REQUEST[salto]");
...
switch (TRUE)
{
  ...
  { # mostra o registro que será excluído e pede confirmação do usuário.
    ...
    mostraregistro("$_REQUEST[cpmedico]",'Confirmar Excluir',FALSE,$_REQUEST['salto']-2,$_REQUEST['salto']-1,"$_REQUEST[salto]");
    printf("</form>");
    break;
  }
  case ($bloco==3):
...
terminapagina("médicos","Excluir","medicosexcluir.php");
?>
